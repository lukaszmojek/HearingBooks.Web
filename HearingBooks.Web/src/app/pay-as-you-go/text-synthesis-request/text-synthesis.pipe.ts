...
export class TextSynthesisPipe implements PipeTransform {
  private characterCountTranslationKey = 'TextSyntheses.CharacterCount'
  private priceTranslationKey = 'TextSyntheses.Price'
  
  private characterCountTranslation$: Observable<string>
  private priceTranslation$: Observable<string>

  constructor(private translate: TranslateService) {
    this.characterCountTranslation$ = this.translate.get(this.characterCountTranslationKey)
    this.priceTranslation$ = this.translate.get(this.priceTranslationKey)
  }

  transform(value: ITextSynthesisRequest, ...args: unknown[]): Observable<string> {
