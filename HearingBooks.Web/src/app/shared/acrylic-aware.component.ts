...
export default abstract class AcrylicAwareComponent extends StoreConnectedComponent<IApplicationState> {
  ...
  constructor(store$: Store<IApplicationState>) {
    super(store$)
    this.safeSelect$(selectMainCardType).subscribe(mainCardType => this.mainCardType = mainCardType)
    this.safeSelect$(selectInnerCardType).subscribe(innerCardType => this.innerCardType = innerCardType)
